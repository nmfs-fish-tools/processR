...
void WriteEnvironmentToSharedMemory(Rcpp::Environment env, std::string name) {
    Rcpp::Environment my_env = copyEnvironment(env); //copy the environment to avoid race conditions.
...
void copyToGlobalnvironment(const Rcpp::Environment& sourceEnv) {
...
int main(int argc, char** argv) {
    ...
    Rcpp::Environment environment = readEnvironmentFromSharedMemory(ss_env_name);
    environment.assign("processR.rank", rank);
    copyToGlobalnvironment(environment);
    Rcpp::Function function = readFunctionFromSharedMemory(ss_fun_name);
